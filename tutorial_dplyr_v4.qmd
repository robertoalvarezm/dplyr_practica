...
subtitle: "Con datos reales de salud — Variables categóricas y valores NA"
...
Si ya conoces R base, sabes que se puede hacer casi todo con él. El problema es que conforme los análisis crecen, el código se vuelve difícil de leer y de mantener.
...
El vocabulario de `dplyr` son **6 verbos** principales. Una vez que los dominas, puedes hacer análisis complejos escribiendo código que casi se lee como español.

```{r, eval=FALSE}
# Instalar si aún no tienes los paquetes (una sola vez):
...
## Dataset: Indicadores de salud con variables categóricas

El archivo `dataset_categorical_NA.csv` contiene registros de individuos con mediciones clínicas, hábitos de vida y características sociodemográficas. Incluye valores `NA` (datos faltantes) — una situación muy frecuente en datos reales de salud.

Cada fila representa un **paciente o participante**. Las columnas se dividen en dos grandes grupos:

**Variables numéricas (mediciones clínicas):**
...
**Variables categóricas (características sociodemográficas):**
...
| `MaritalStatus` | Estado civil | `Casada`, `Divorciada`, `Unión libre`, etc. |
| `EmploymentStatus` | Situación laboral | `Tiempo completo`, `Estudiante`, etc. |
...
> 💡 **Nota sobre los NA:** Los valores faltantes son una realidad del trabajo con datos de salud. `dplyr` los respeta y los propaga en los cálculos. Cuando sea necesario ignorarlos usaremos `na.rm = TRUE` dentro de las funciones de resumen.
...
En R base, aplicar varias operaciones seguidas obliga a anidar funciones — hay que leerlas de adentro hacia afuera:
...
> En *Tools → Global Options → Code* activa *"Use native pipe operator (`|>`)"*.
...
`filter()` te deja solo con las filas que cumplen una condición. Es el equivalente de `[condición, ]` en R base, pero más legible.
...
Nota que dentro de `filter()` no necesitas escribir `salud$SmokingStatus` — R ya sabe a qué tabla te refieres.
...
La coma dentro de `filter()` equivale al `&` — ambas condiciones deben cumplirse:
...
```{r}
# Participantes con presión arterial alta O colesterol alto
...
Cuando quieres varios valores de la misma columna, `%in%` es más limpio que encadenar `|`:

```{r}
# Participantes con educación universitaria o de posgrado
...
```{r}
# Incluir explícitamente los NA en un filtro
...
> ⚠️ **Error frecuente:** `=` vs `==`  
> ```r
> filter(salud, SmokingStatus = "Fuma")   # ❌ Error: = es asignación
> filter(salud, SmokingStatus == "Fuma")  # ✅ Correcto: == es comparación
> ```
...
**1.** ¿Cuántos participantes son ex-fumadores?

**2.** Filtra los participantes con nivel de estrés mayor a 80. ¿Qué estatus de tabaquismo predomina entre ellos?

**3.** ¿Cuántos participantes tienen más de 40 años Y residen en zona urbana?  
...
```{r}
# Solo columnas de texto (categóricas)
salud |>
...
Patrón muy común: filtra las filas de interés, luego quédate con las columnas relevantes:
...
**1.** Selecciona solo las columnas categóricas (`where(is.character)`) del dataset `salud`.
...
**3.** Filtra solo los participantes de zona rural y luego selecciona únicamente `ID`, `Age`, `BMI` y `SmokingStatus`.
...
`arrange()` ordena las filas según los valores de una o varias columnas. Por defecto: menor a mayor. Para invertir: `desc()`.
...
```{r}
# ¿Quiénes tienen mayor nivel de colesterol?
salud |>
...
La segunda columna actúa como desempate:
...
**1.** ¿Qué participante tiene el nivel de glucosa más alto? Muestra el top 5.
...
`mutate()` añade columnas nuevas calculadas a partir de las que ya existen, o modifica las existentes. No quita ni agrega filas.
...
> 💡 `mutate()` **no modifica el objeto original**. Para guardar los cambios: `salud <- salud |> mutate(...)`
...
salud <- salud |>
  mutate(
    # Índice combinado de riesgo cardiovascular (simplificado)
...
`case_when()` es como un `if-else` encadenado. La sintaxis: `condición ~ valor`. El `TRUE` al final actúa como "else":
...
> ⚠️ **Importante con NA:** Si no manejas los `NA` explícitamente en `case_when()` como primera condición (`is.na(...) ~ "Sin dato"`), los registros sin dato caerán en el `TRUE` y se clasificarán incorrectamente.

## Clasificar el estrés por niveles
...
## Transformaciones matemáticas útiles

```{r}
# Centrar la edad respecto a la media del grupo (útil para análisis)
salud |>
  filter(!is.na(Age)) |>
  mutate(
...
**3.** Crea una columna `imc_normalizado` que calcule cuántos puntos de BMI tiene cada participante por encima o por debajo de la media del grupo.
...
`summarise()` **colapsa** todo el data frame a una sola fila de estadísticas. Es como hacer `mean()`, `sum()`, etc. en R base, pero de forma organizada.
...
# DPLYR: más organizado y extendible
salud |>
  summarise(
    n               = n(),
...
Esta es la combinación más poderosa de `dplyr`. `group_by()` pone separadores invisibles en la tabla; `summarise()` calcula estadísticos **dentro de cada grupo**.

Piénsalo como carpetas: `group_by()` pone las divisiones, `summarise()` saca las cuentas de cada sección.

```{r}
# R BASE: promedio de BMI por estatus de tabaquismo
tapply(salud$BMI, salud$SmokingStatus, mean, na.rm = TRUE)

# DPLYR: más claro y fácilmente extendible
salud |>
  ...
  summarise(
    ...
    .groups       = "drop"    # siempre añadir: limpia los grupos residuales
  )
```

> ⚠️ **Buena práctica:** siempre añade `.groups = "drop"` al final de `summarise()` cuando usas `group_by()`. Si no, el resultado sigue agrupado y puede causar comportamientos inesperados.
...
## Análisis real: ¿Los fumadores tienen mayor riesgo cardiovascular?
...
perfil_educacion <- salud |>
  ...
  summarise(
    ...
    media_sueño     = round(mean(SleepHours, na.rm = TRUE), 1),
    .groups         = "drop"
  ) |>
...
**1.** ¿Cuál es el promedio de horas de sueño por tipo de residencia?

**2.** ¿Qué estado civil tiene el nivel de estrés más alto en promedio?

**3.** Calcula el promedio de glucosa y colesterol por estatus de tabaquismo.

**4. Desafío:** ¿En qué combinación de `EducationLevel` y `ResidenceType` se observa el mayor IMC promedio?
...
```{r}
# 1. Horas de sueño por residencia
salud |>
  group_by(ResidenceType) |>
  summarise(media_sueño = round(mean(SleepHours, na.rm = TRUE), 1), .groups = "drop")
...
# 4. Educación + residencia con mayor IMC
salud |>
...
Muchas veces la información está en tablas separadas. Los **joins** las unen usando una columna llave en común.
...
Ahora podemos calcular si cada participante está por encima o por debajo del promedio de su zona:
...
| `anti_join` | Filas de la izquierda SIN coincidencia | Quieres encontrar qué falta o qué no coincide |

> 💡 **`left_join()`** es el join más usado: no pierdes ninguna fila de tu tabla principal.

---

# Funciones de ventana

Las funciones de ventana calculan un valor por fila usando información de **otras filas del mismo grupo**, sin colapsar. Son ideales para rankings y comparaciones.

## Ranking dentro de grupos: `dense_rank()`

```{r}
# ¿Quiénes tienen el mayor IMC dentro de cada grupo de tabaquismo?
salud |>
...
## `lag()` — Comparar registros consecutivos
...
```{r}
# Promedio de TODAS las variables numéricas agrupado por estatus de tabaquismo
salud |>
  filter(!is.na(SmokingStatus)) |>
  group_by(SmokingStatus) |>
  summarise(
...
```{r}
# ¿Cuántos niveles educativos distintos hay?
salud |>
...
# Caso integrador: análisis completo del dataset de salud

Combinamos todo lo aprendido para responder preguntas concretas con datos reales.

## Pregunta 1: ¿Existen diferencias de riesgo cardiovascular entre tipos de residencia?
...
## Pregunta 2: ¿Cómo se distribuye el IMC por tabaquismo y nivel educativo?
...
| `summarise()` | Resume a estadísticos | `mean()`, `sum()`, etc. |
...
Usando el dataset `salud`, responde las siguientes preguntas **en un solo bloque de código encadenado con pipes**:

**A)** ¿Cuál es el promedio de estrés y horas de sueño por tipo de residencia y estado civil? Ordena de mayor a menor estrés. Incluye solo grupos con al menos 1 observación.

**B)** ¿Quiénes son los 5 participantes con mayor diferencia entre su IMC y el IMC promedio de su grupo educativo? Muestra su ID, educación, IMC y la diferencia.
...
```{r}
# A) Estrés y sueño por residencia y estado civil
salud |>
...
# B) Top 5 con mayor desvío de IMC respecto a su grupo educativo
salud |>
...
# Próximos pasos {.unnumbered}

Una vez que domines estos verbos, el camino natural dentro del ecosistema tidyverse es:

- **`tidyr`** — transformar la forma de las tablas (`pivot_wider`, `pivot_longer`)
- **`stringr`** — manipular texto y cadenas de caracteres
- **`lubridate`** — trabajar con fechas y tiempos
- **`ggplot2`** — visualización de datos (que ya usamos en este tutorial)

Todo con la misma filosofía: código que se lee de arriba a abajo, verbos expresivos y pipes que conectan las operaciones.
